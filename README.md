...
`Organizer`
+ The user or entity that creates and manages an specific voting `process`
...
+ The `sk-snarks circuit` will use its merkle-root to validate if a `voter` `public key` is part of it

`Zk-snarks proof`
+ The `zk-snarks proof` is used prove two things without revealing critical data
...
+ It is validated by the `oranizer` once the `process` ends
...
### 5. Finalizing the `process`
  + The owners of the `vote encryption keys` publish the corresponding `private keys`, so the votes and proofs can ve decrypted
...
- IP/vote mapping
- Most of the unresolved details are around creating a fully decentralized relay network. Multiple alternative exist.
- A centralized trusted `relay` is a very valid option in certain context
